...
static bool ReportingUpdate = false;
...
ZStatus_t bdb_getReport( uint8_t endpoint, uint16_t cluster, uint16_t attrID, bdbReportAttrCfgData_t* report, bool force) {
...
//    zclCfgReportRspCmd_t *cfgReportRspCmd;
    ...
    if (ReportingUpdate || force) {
        ReportingUpdate = false;
        // Find Ep Descriptor
        endPointDesc_t* epDescriptor = bdb_FindEpDesc( endpoint );
...
//          OsalPort_free( cfgReportRspCmd );
        ...
        uint8_t atrrCfgRecordIndex =  bdb_repAttrCfgRecordsArraySearch( endpoint, cluster, attrID );
        uint8_t status2 = zclFindAttrRec( endpoint, cluster, attrID, &attrRec );
        ...
        {
            // found the resource
//            report->minReportInt = bdb_reportingAttrCfgRecordsArray[atrrCfgRecordIndex].minReportInt;
//            report->maxReportInt = bdb_reportingAttrCfgRecordsArray[atrrCfgRecordIndex].maxReportInt;
            // For attributes of 'discrete' data types this field is omitted
//            memset( report->reportableChange, 0x00, BDBREPORTING_MAX_ANALOG_ATTR_SIZE );
//            OsalPort_memcpy( report->reportableChange, bdb_reportingAttrCfgRecordsArray[atrrCfgRecordIndex].reportableChange, BDBREPORTING_MAX_ANALOG_ATTR_SIZE );
            memset( report, 0x00, sizeof(bdbReportAttrCfgData_t) );
            OsalPort_memcpy( report, &bdb_reportingAttrCfgRecordsArray[atrrCfgRecordIndex], sizeof(bdbReportAttrCfgData_t) );
